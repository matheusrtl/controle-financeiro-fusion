...
import { Upload, History, Users, LogOut, LayoutDashboard, Lock } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const roleLoading = adminQ.isLoading;
        ...
        <header className="sticky top-0 z-30 border-b border-border/70 bg-card/95 backdrop-blur">
          <div className="mx-auto flex max-w-[1600px] items-center gap-6 px-4 py-3 sm:px-6">
            ...
            <nav className="ml-auto flex items-center gap-1">
              ...
              <NavItem to="/upload" icon={<Upload className="h-4 w-4" />} label="Upload" disabled={!roleLoading && !isAdmin} />
              <NavItem to="/historico" icon={<History className="h-4 w-4" />} label="Histórico" />
              <NavItem to="/usuarios" icon={<Users className="h-4 w-4" />} label="Usuários" disabled={!roleLoading && !isAdmin} />
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            aria-disabled="true"
            className="inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground/50 cursor-not-allowed select-none"
          >
            {icon}<span className="hidden sm:inline">{label}</span>
            <Lock className="h-3 w-3" />
          </span>
        </TooltipTrigger>
        <TooltipContent>Somente administradores</TooltipContent>
      </Tooltip>
    );
  }
  return (
